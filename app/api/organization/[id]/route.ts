...
export async function PUT(request: Request, { params }: { params: any }) {
  try {
    ...
    const servantsFromRooms = updatedOrganization.rooms.flatMap(
      (room) => room.servants
    );
  ...
  try {
    ...
    if (existingOrg.children.length > 0) {
      return NextResponse.json(
        {
          error:
            "Cannot delete organization with enrolled children. Please reassign children first.",
        },
        { status: 400 }
      );
    }
    ...
    if (existingOrg.rooms.length > 0) {
      return NextResponse.json(
        {
          error:
            "Cannot delete organization with assigned rooms. Please reassign rooms first.",
        },
        { status: 400 }
      );
    }
