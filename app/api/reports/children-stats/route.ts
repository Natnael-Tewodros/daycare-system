...
function getYearRanges(year?: number) {
  const now = new Date();
  const y = typeof year === 'number' && !Number.isNaN(year) ? year : now.getFullYear();
...
export async function GET(request: NextRequest) {
  ...
  const { thisYearStart, nextYearStart, lastYearStart, lastYearEnd, year } = getYearRanges(y);
    ...
    const orgIds = byOrgGroup.map(g => g.organizationId).filter((v): v is number => v != null);
    const orgs = await prisma.organization.findMany({
      where: { id: { in: orgIds } },
      select: { id: true, name: true }
    });
    const orgIdToName = new Map(orgs.map(o => [o.id, o.name]));
    const byOrganization = byOrgGroup.map(g => ({
      organization: g.organizationId ? (orgIdToName.get(g.organizationId) || 'Unknown') : 'Unassigned',
      count: g._count._all
    ...
    const siteIds = bySiteGroup.map(g => g.siteId).filter((v): v is number => v != null);
    const sites = await prisma.site.findMany({
      where: { id: { in: siteIds } },
      select: { id: true, name: true }
    });
    const siteIdToName = new Map(sites.map(s => [s.id, s.name]));
    const bySite = bySiteGroup.map(g => ({
    ...
    return NextResponse.json({ error: 'Failed to compute children stats' }, { status: 500 });
  }
}
