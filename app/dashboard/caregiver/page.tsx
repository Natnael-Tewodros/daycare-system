...
export default function CaregiversPage() {
  ...
  useEffect(() => {
    Promise.all([
      fetch("/api/caregivers/caregivers")
        .then((r) => r.json())
        .then(setServants),
      fetch("/api/rooms")
        .then((r) => r.json())
      ...
      fetch("/api/children")
        ...
        .then((data) =>
          setChildren(
            data.map((c) => ({
              ...c,
              assignedServantId: c.servant?.id ?? c.caregiver?.id ?? null,
            }))
          )
        ),
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (responseData.details && typeof responseData.details === "object") {
          ...
          setErrors({
            submit: `Server error: ${res.status} ${
              res.statusText || "Unknown error"
            }. Please try again.`,
          });
        }
      ...
      setOpenEdit(false);
      resetForm();
      const updatedCaregivers = await fetch("/api/caregivers").then((r) =>
        r.json()
      );
  ...
  const updateRoom = async (id, roomId) => {
    try {
      const res = await fetch(`/api/caregivers/${id}`, {
        ...
        body: JSON.stringify({
          assignedRoomId: roomId === "none" ? null : +roomId,
        }),
      });

      if (!res.ok) {
        const err = await res.json().catch(() => ({}));
        alert(err.error || err.message || "Failed to transfer caregiver");
        return;
      }
      ...
      setServants((s) =>
        s.map((x) =>
          x.id === id
            ? { ...x, assignedRoomId: roomId === "none" ? null : +roomId }
            : x
        )
      );
      ...
      alert("Failed to transfer caregiver");
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600 mt-1">
                Manage caregivers and their assignments
              </p>
            </div>
          </div>
        </div>
            ...
            <CardContent className="p-6">
              ...
              <div className="grid md:grid-cols-2 gap-3">
                {children
                  ...
                  .slice(0, 8)
                  .map((child) => {
                    const caregiver = servants.find(
                      (s) =>
                        s.id?.toString() === child.assignedServantId?.toString()
                    );
                    return (
                      <div
                        key={child.id}
                      ...
                      >
                        <div>
                          ...
                          <div className="text-xs text-gray-500">
                            Caregiver: {caregiver?.fullName || "Unassigned"}
                          </div>
                        </div>
                        <Badge
                          variant={caregiver ? "default" : "secondary"}
                ...
                <tbody className="divide-y divide-gray-200">
                  {paginated.map((servant) => {
                    const assignedChildren = children.filter(
                      (c) => c.assignedServantId === servant.id
                    );
                    return (
                      <tr key={servant.id} className="hover:bg-gray-50">
                        ...
                        <TableCell>
                          <Select
                            ...
                            onValueChange={(v) => updateRoom(servant.id, v)}
                          >
                        ...
                        <TableCell>
                          <div className="space-y-2">
                            ...
                            {assignedChildren.length > 0 && (
                              <div className="text-xs text-gray-500">
                                {assignedChildren.slice(0, 2).map((child) => (
                                  <div key={child.id}>• {child.fullName}</div>
                                ))}
                                {assignedChildren.length > 2 && (
                                  <div className="text-gray-400">
                                    +{assignedChildren.length - 2} more
                                  </div>
                                )}
                              </div>
                            )}
                            ...
                            <Button
                              ...
                              onClick={() => {
                                ...
                                setForm({
                                  ...
                                  phone: servant.phone ?? "",
                                  assignedRoomId:
                                    servant.assignedRoomId?.toString() ??
                                    "none",
                                  site: servant.site ?? "",
                                  organizationType:
                                    servant.organizationType ?? "",
      ...
      <Dialog
        ...
        onOpenChange={openEdit ? setOpenEdit : setOpenCreate}
      >
        <DialogContent className="sm:max-w-lg">
          ...
          <CaregiverForm
            ...
            currentReportLabel={
              selected?.medicalReportFile ? "Update Medical Report" : ""
            }
          />
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={openTransfer} onOpenChange={setOpenTransfer}>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4">
            <Select
              value={transfer.caregiverId}
              onValueChange={(v) =>
                setTransfer((t) => ({ ...t, caregiverId: v }))
              }
            >
              <SelectTrigger>
            ...
            <Select
              value={transfer.roomId}
              onValueChange={(v) => setTransfer((t) => ({ ...t, roomId: v }))}
            >
              <SelectTrigger>
                <SelectValue placeholder="Select room" />
              </SelectTrigger>
            ...
            <Button
              onClick={() => updateRoom(+transfer.caregiverId, transfer.roomId)}
              className="w-full"
            >
              <ArrowRightLeft className="h-4 w-4 mr-2" />
              Transfer
      ...
      <Dialog open={openAssign} onOpenChange={setOpenAssign}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>Assign Children</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <Select
              value={assign.caregiverId}
              onValueChange={(v) =>
                setAssign((a) => ({ ...a, caregiverId: v }))
              }
            >
              <SelectTrigger>
                <SelectValue placeholder="Select caregiver" />
            ...
            <div className="max-h-60 overflow-y-auto border rounded p-3 space-y-2 bg-gray-50">
              {children.map((child) => (
                ...
                >
                  <input
                    type="checkbox"
                    checked={assign.selectedChildren.includes(child.id)}
                    onChange={(e) =>
                      setAssign((a) => ({
                        ...a,
                        selectedChildren: e.target.checked
                          ? [...a.selectedChildren, child.id]
                          : a.selectedChildren.filter((id) => id !== child.id),
                      }))
                    }
                  />
                  <div>
            ...
            <Button onClick={assignChildren} className="w-full">
              <UserCheck className="h-4 w-4 mr-2" />
              Assign Children
            </Button>
          </div>
